  ...
  return (
    <div className="container max-w-full max-h-full  flex flex-col flex-grow p-6 mx-auto">
      <div className="flex justify-between items-center mb-2">
        <h1 className="text-xl font-bold text-gray-900">Account Cashflows</h1>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`bg-[#E95900] text-white px-4 py-1.5 rounded font-semibold hover:bg-blue-700 text-sm transition-colors ${running ? 'opacity-60 cursor-not-allowed' : ''}`}
          >
            Run
          </button>
          {data && (
      ...
      {running && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
          <div className="bg-white rounded-lg shadow-lg p-8 flex flex-col items-center">
            ...
            <span className="text-blue-600 font-medium text-lg">Running cashflows...</span>
          </div>
        </div>
      )}
      {showModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
          <div className="bg-white rounded-lg shadow-lg p-8 flex flex-col items-center">
            <span className="text-green-600 font-bold text-lg mb-4">Cashflows done</span>
            <button
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full border border-gray-200 rounded-md px-3 py-2 pr-10"
          ...
          <div className="bg-white rounded-lg ">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                ...
                <p className="font-medium">{data.loan.v_account_number}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">MIS Date</p>
                <p className="font-medium">{formatDate(data.loan.fic_mis_date)}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Currency</p>
                <p className="font-medium">{data.loan.currency}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Loan Amount</p>
                <p className="font-medium">{formatCurrency(data.loan.loan_amount, data.loan.currency)}</p>
              </div>
            </div>
          </div>

          {/* Summary */}
          <div className="bg-white rounded-lgbg">
            <h2 className="text-lg font-semibold mb-4">Cashflow Summary</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                <p className="text-sm text-gray-500">Total Cashflows</p>
                <p className="font-medium">{data.summary.total_cashflows}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Total Principal</p>
                <p className="font-medium">{formatCurrency(data.summary.total_principal, data.summary.currency)}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Total Interest</p>
                <p className="font-medium">{formatCurrency(data.summary.total_interest, data.summary.currency)}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Total Amount</p>
                <p className="font-medium">{formatCurrency(data.summary.total_amount, data.summary.currency)}</p>
              </div>
            </div>
          </div>
              ...
              <table className="w-full min-w-max border-collapse">
                <thead className="sticky top-0 bg-white">
                  <tr className="bg-gray-100">
                    <th className="px-4 py-3 text-left text-xs font-bold text-black border-b border-gray-200" style={{ minWidth: "80px" }}>
                      Bucket
                    </th>
                    <th className="px-4 py-3 text-left text-xs font-bold text-black border-b border-gray-200" style={{ minWidth: "120px" }}>
                      Date
                    </th>
                    <th className="px-4 py-3 text-left text-xs font-bold text-black border-b border-gray-200" style={{ minWidth: "120px" }}>
                      Principal
