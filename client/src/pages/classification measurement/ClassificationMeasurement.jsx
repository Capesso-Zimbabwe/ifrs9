    ...
    <div className="container max-w-full max-h-full overflow-hidden flex flex-col flex-grow p-6 mx-auto">
      <div className="flex justify-between items-center mb-2">
        ...
        {selectedRows.length > 0 && editedClassification && (
          <button 
            className="px-4 py-2 bg-gray-500 text-white text-sm rounded-md hover:bg-gray-600 transition-colors duration-200 shadow-sm flex items-center gap-2"
            onClick={handleSave}
          >
            <svg 
              className="w-4 h-4" 
          ...
          <table className="w-full min-w-max border-collapse">
            <thead className="sticky top-0 bg-white">
              <tr className="bg-gray-100">
                <th className="px-4 py-3 text-left text-xs font-bold text-black border-b border-gray-200 w-10"></th>
                {columns.map((column) => (
                  <th
                    key={column.id}
                ...
                return (
                  ...
                  >
                    <td className="px-4 py-2 text-xs text-gray-900 whitespace-nowrap border-x border-gray-100">
                      <input
                        type="checkbox"
                        className="rounded border-gray-50 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                        checked={isSelected}
        ...
        <div className="flex justify-between items-center p-4 border-t border-gray-200">
          <button
            ...
            className={`px-3 py-1 rounded ${
              page === 1 || tableLoading ? 'bg-gray-100 text-gray-400' : 'bg-blue-500 text-white hover:bg-blue-600'
            }`}
          >
            Previous
          </button>
          ...
          <button
            ...
            className={`px-3 py-1 rounded ${
              page === totalPages || tableLoading ? 'bg-gray-100 text-gray-400' : 'bg-[#E95900] text-white hover:bg-blue-600'
            }`}
          >
            Next
          </button>
