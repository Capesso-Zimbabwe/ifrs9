            ...
            <div className="space-y-4">
              ...
              {isRunning && (
                <div className="bg-blue-50 border-l-4 border-blue-500 p-3 flex items-center">
                  ...
                  <p className="text-xs text-blue-700">ECL calculation is in progress...</p>
                </div>
              )}
              ...
              {progress.length > 0 ? (
                <div className="overflow-auto rounded-md border border-gray-200">
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-100">
                      <tr>
                        <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Process</th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-100">
                      {progress.map((step, index) => (
                        <tr key={index} className={index === progress.length - 1 && isRunning ? 'bg-blue-50' : ''}>
                          <td className="px-3 py-2 text-xs text-gray-800">{step.step}</td>
                          <td className="px-3 py-2 text-xs">
                            <span
                              className={`px-2 py-0.5 rounded-full text-xs font-medium ${getStatusColor(step.status)}`}
                            >
                ...
                <div className="text-center py-8">
                  ...
                  <h3 className="mt-2 text-sm font-medium text-gray-900">No calculation in progress</h3>
    ...
    <div className="w-full px-5 mt-4 min-h-screen ">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-lg font-bold text-gray-800">ECL Calculation</h1>
          <div className="flex items-center space-x-3">
            <div className="flex items-center space-x-2">
              ...
              <input
                id="ficMisDate"
                type="date"
                className="border border-gray-300 rounded-md px-2.5 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500"
                value={ficMisDate}
            ...
            {isRunning ? (
              <button
                className="px-3 py-1.5 rounded-md text-xs text-white font-medium bg-red-600 hover:bg-red-700 transition-colors"
              ...
              <button
                className="px-3 py-1.5 rounded-md text-xs text-white font-medium bg-[#E95900] hover:bg-blue-700 transition-colors"
        ...
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl mx-auto overflow-hidden">
            <div className="flex justify-between items-center px-4 py-3 bg-gray-50 border-b border-gray-200">
              <h3 className="text-sm font-medium text-gray-900">{modalContent.title}</h3>
              <button
            ...
            <div className="px-4 py-3 bg-gray-50 border-t border-gray-200 flex justify-end">
              <button
                onClick={() => setShowModal(false)}
                className="px-3 py-1.5 bg-blue-600 text-xs text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
              >
                Close
              </button>
            </div>
