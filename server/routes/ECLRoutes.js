...
router.get('/run-simple-ead/stream', (req, res) => {
  ...
  // Start the process
  (async () => {
    const runKey = await getLatestRunKey();
    if (!runKey) {
        sendEvent({ 
            step: 'ECL calculation failed', 
            status: 'Error',
            output: 'Failed to get run key from dim_run table'
        });
        res.end();
        return;
    }
    await upsertEclRun(runKey, date, 'Running', false);
    try {
      await runAllScripts();
      await upsertEclRun(runKey, date, 'Success', false);
    } catch (err) {
    ...
    sendEvent({ 
      ...
      await upsertEclRun(runKey, date, 'Failed', false);
    }
  })();
...
router.post('/run-simple-ead', async (req, res) => {
  ...
  const runKey = await getLatestRunKey();
  if (!runKey) {
    return res.status(500).json({ error: 'Failed to get run key from dim_run table' });
  }
  await upsertEclRun(runKey, date, 'Running', false);
...
router.post('/approve/:runKey', async (req, res) => {
  ...
  try {
    connection = await db.getConnection();
    const query = 'UPDATE ecl_runs SET approved = ? WHERE run_key = ?';
    await connection.execute(query, [approved, runKey]);
    res.json({ success: true, message: 'ECL run approval status updated' });
  } catch (err) {
